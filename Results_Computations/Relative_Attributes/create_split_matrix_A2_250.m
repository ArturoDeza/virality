...
for k=1:1
	...
	split_A2{k}{1}.ClassName = './positive';
	split_A2{k}{2}.ClassName = './negative';
	split_A2{k}{3}.ClassName = './neutral';
	...
	for i=1:5039
		...
		%if A_10(k,i)==1
			...
			if Attribute_Matrix(i,2) == 1
				if virality_label(i) == 1
					split_A2{k}{1}.Training{j_p_train} = img_str_jpg;
					split_A2{k}{1}.TrainingID{j_p_train} = img_id;
					j_p_train = j_p_train + 1;
				elseif virality_label(i) == 0
					split_A2{k}{2}.Training{j_n_train} = img_str_jpg;
					split_A2{k}{2}.TrainingID{j_n_train} = img_id;
			...
			elseif Attribute_Matrix(i,2) == 0
				split_A2{k}{3}.Training{j_0_train} = img_str_jpg;
				split_A2{k}{3}.TrainingID{j_0_train} = img_id;
			...
			if Attribute_Matrix(i,2) == 1
				if virality_label(i) == 1
					split_A2{k}{1}.Testing{j_p_test} = img_str_jpg;
					split_A2{k}{1}.TestingID{j_p_test} = img_id;
					j_p_test = j_p_test + 1;			
				elseif virality_label(i) == 0
					split_A2{k}{2}.Testing{j_n_test} = img_str_jpg;
					split_A2{k}{2}.TestingID{j_n_test} = img_id;
					j_n_test = j_n_test + 1;
				else
					%Do nothing
				end		
			...
			% If element is neutral
			elseif Attribute_Matrix(i,2) == 0
				split_A2{k}{3}.Testing{j_0_test} = img_str_jpg;
				split_A2{k}{3}.TestingID{j_0_test} = img_id;
				j_0_test = j_0_test + 1;
...
%End of everything
save('./split_A2_Top.mat','split');
save('./synthgen_N/split_A2_Top.mat','split');
